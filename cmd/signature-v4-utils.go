...
func isValidRegion(reqRegion string, confRegion string) bool {
	...
	if confRegion == "US" {
		confRegion = globalRadioDefaultRegion
	}
	// Some older s3 clients set region as "US" instead of
	// globalRadioDefaultRegion, handle it.
	if reqRegion == "US" {
		reqRegion = globalRadioDefaultRegion
	}
...
func checkKeyValid(accessKey string) (auth.Credentials, APIErrorCode) {
	cred, ok := globalLocalCreds[accessKey]
	if !ok {
