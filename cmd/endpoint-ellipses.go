...
func createServerEndpoints(serverAddr string, args ...string) (EndpointZones, error) {
	if len(args) == 0 {
		return nil, errInvalidArgument
	}
	...
	if !ellipses.HasEllipses(args...) {
		...
		endpointList, err := CreateEndpoints(serverAddr, setArgs...)
		if err != nil {
			return nil, err
		}
	...
	// Verify the args setup-type appropriately.
	{
		...
		_, err = CreateEndpoints(serverAddr, setArgs...)
		if err != nil {
			return nil, err
		}
	}
