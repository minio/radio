...
type ServerSystemConfig struct {
	RadioPlatform  string
	RadioRuntime   string
	RadioEndpoints EndpointZones
}

// Diff - returns error on first difference found in two configs.
func (s1 ServerSystemConfig) Diff(s2 ServerSystemConfig) error {
	if s1.RadioPlatform != s2.RadioPlatform {
		return fmt.Errorf("Expected platform '%s', found to be running '%s'",
			s1.RadioPlatform, s2.RadioPlatform)
	}
	if s1.RadioEndpoints.Nodes() != s2.RadioEndpoints.Nodes() {
		return fmt.Errorf("Expected number of endpoints %d, seen %d", s1.RadioEndpoints.Nodes(),
			s2.RadioEndpoints.Nodes())
	}

	for i, ep := range s1.RadioEndpoints {
		if ep.SetCount != s2.RadioEndpoints[i].SetCount {
			return fmt.Errorf("Expected set count %d, seen %d", ep.SetCount,
				s2.RadioEndpoints[i].SetCount)
		}
		if ep.DrivesPerSet != s2.RadioEndpoints[i].DrivesPerSet {
			return fmt.Errorf("Expected drives pet set %d, seen %d", ep.DrivesPerSet,
				s2.RadioEndpoints[i].DrivesPerSet)
		}
		for j, endpoint := range ep.Endpoints {
			if endpoint.String() != s2.RadioEndpoints[i].Endpoints[j].String() {
				return fmt.Errorf("Expected endpoint %s, seen %s", endpoint,
					s2.RadioEndpoints[i].Endpoints[j])
			}
		}
...
func getServerSystemCfg() ServerSystemConfig {
	return ServerSystemConfig{
		RadioPlatform:  fmt.Sprintf("OS: %s | Arch: %s", runtime.GOOS, runtime.GOARCH),
		RadioEndpoints: globalEndpoints,
	}
}
...
func newBootstrapRESTClient(endpoint Endpoint) (*bootstrapRESTClient, error) {
	...
	restClient, err := rest.NewClient(serverURL, trFn, func() string { return "" })
	if err != nil {
		return nil, err
	}
