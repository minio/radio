...
var errorCodes = errorCodeMap{
	...
	ErrUnsupportedNotification: {
		...
		Description:    "Radio server does not support Topic or Cloud Function based notifications.",
		HTTPStatusCode: http.StatusBadRequest,
	},
	ErrInvalidCopyPartRange: {
		Code:           "InvalidArgument",
...
func toAPIErrorCode(ctx context.Context, err error) (apiErr APIErrorCode) {
	...
	// SSE errors
	case ErrMissingCustomerKey:
		apiErr = ErrMissingSSECustomerKey
	case ErrMissingCustomerKeyMD5:
		apiErr = ErrMissingSSECustomerKeyMD5
	case ErrCustomerKeyMD5Mismatch:
		apiErr = ErrSSECustomerKeyMD5Mismatch
	case ErrInvalidCustomerKey, ErrSecretKeyMismatch:
...
func toAPIError(ctx context.Context, err error) APIError {
	...
	if apiErr.Code == "InternalError" {
		...
		// useful with radio implementations.
		switch e := err.(type) {
