...
func doesPolicySignatureV2Match(formValues http.Header) APIErrorCode {
	...
	cred, s3Err := checkKeyValid(accessKey)
	if s3Err != ErrNone {
		return s3Err
	}
...
func getReqAccessKeyV2(r *http.Request) (auth.Credentials, APIErrorCode) {
	...
	if len(authFields) != 2 {
		return auth.Credentials{}, ErrMissingFields
	}
...
func validateV2AuthHeader(r *http.Request) (auth.Credentials, APIErrorCode) {
	...
	cred, apiErr := getReqAccessKeyV2(r)
	if apiErr != ErrNone {
		return cred, apiErr
	}
